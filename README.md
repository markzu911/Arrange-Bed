...
1. 上传素材：上传卧室照片和床具照片，前端压缩为适合接口传输的 JPEG。
2. 场景解析：开始图片分析前调用 `/api/tool/verify` 校验积分。
3. 分析确认：展示结构化分析内容，用户可编辑。
4. 摆放设置：选择摆放位置、多视角、尺寸、模型、清晰度、融合强度和备注。
...
iframe.contentWindow?.postMessage({
  type: "SAAS_INIT",
  userId: "user_123",
  toolId: "villa-bedding-placement",
  context: "平台传入上下文",
  prompt: ["卧室床具", "真实卧室软装"],
...
工具也支持 URL 查询参数兜底：`?userId=user_123&toolId=villa-bedding-placement`。

## 文件结构

```text
api/proxy.ts                         Vercel 服务端入口
src/VillaBeddingPlacementTool.tsx     工具主组件
src/VillaBeddingPlacementTool.module.css 局部样式
