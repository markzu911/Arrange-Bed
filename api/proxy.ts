...
async function generateImagesWithInteractions(body: GeminiRequestBody, apiKey: string, model: string) {
  ...
  await Promise.all(requested.filter((item) => item !== "wide").map(async (perspective) => {
    ...
    results.push({ perspective, title: perspective === "medium" ? "中近景（床区核心）" : "近景（床具细节）", imageUrl: `data:${variationImage.mimeType};base64,${variationImage.data}` });
  }));
...
function createMockGeminiResponse(body: GeminiRequestBody) {
  if (body.mode === "analyze") {
    return {
      success: true,
      analysis: {
        roomSummary: "卧室空间、地面与床头墙关系清晰，可结合门、窗、衣柜和主要通道规划床位。",
        beddingSummary: "床具主体清晰，应保留原有颜色、材质质感、床头造型、床架轮廓和床垫厚度。",
        beddingIdentity: {
          sizeSense: "以床具参考图为准",
          silhouette: "以床具参考图整体轮廓为准",
          headboard: "以床头造型为准",
          frame: "以床架轮廓为准",
          mattress: "以床垫厚度为准",
          material: "以可见材质纹理为准",
          color: "以主色为准",
          details: ["保留软包、纹理、缝线、拉扣、床脚和可见配件"]
        },
        lighting: "自然光从侧向进入，生成时需要补充地面接触阴影和环境反射。",
        perspective: "卧室具备明显纵深，床具应随地面透视线调整尺度和角度。",
        placementAdvice: "建议优先让床头靠实墙，避开门、窗、衣柜和主要通行动线。",
        constraints: ["不要改变卧室主体结构", "不要添加人物和文字", "保持原床具材质与比例"],
        placementPlan: {
          summary: "将目标床具自然摆放到卧室核心床区，优先保证床头墙、采光和通行动线合理。",
          placement: "由 AI 结合床头墙、门窗衣柜和动线选择最合适的位置。",
          facing: "床头靠向合适实墙。",
          scale: "按地面透视和周边家具尺度匹配。",
          preserve: ["保留未被明确要求移除的结构、家具和装饰"],
          remove: ["无明确移除对象"],
          avoid: ["不要遮挡门、窗、衣柜、主要通道和床头柜/插座位置"],
          rationale: ["优先保证床区使用舒适与空间动线完整"],
          candidates: [
            { id: "candidate-main", label: "床头墙方案", placement: "床头靠实墙，避开门窗衣柜和进出通道", facing: "床头靠墙，床尾朝向开阔区", scale: "与地面透视协调", score: 0.9, reasons: ["通道完整", "床头稳定"], blocksWalkway: false, conflictsWithPreservedItems: false, violatesUserRequirements: false },
            { id: "candidate-window-side", label: "侧窗方案", placement: "靠近侧墙摆放，保留窗前采光和通道", facing: "床头靠侧墙", scale: "略小于主床头墙方案", score: 0.65, reasons: ["可保留中央活动区"], blocksWalkway: false, conflictsWithPreservedItems: false, violatesUserRequirements: false },
            { id: "candidate-blocked", label: "通道阻塞方案", placement: "靠近出入口摆放", facing: "床尾朝向衣柜", scale: "偏大", score: 0.8, reasons: ["会影响通行"], blocksWalkway: true, conflictsWithPreservedItems: false, violatesUserRequirements: false }
          ],
...
function parseAnalysis(data: unknown) {
  ...
  try {
    ...
    return {
      ...
      placementPlan: parsePlacementPlan(null, "根据卧室空间、床头墙和通行动线自然摆放目标床具。")
    };
  }
}
...
function parsePlacementPlan(value: unknown, fallbackAdvice: unknown) {
  ...
  const fallback = toReadableText(fallbackAdvice, "根据卧室空间、床头墙和通行动线自然摆放目标床具。");
  return {
    summary: toReadableText(plan.summary, fallback),
    placement: toReadableText(plan.placement, "由 AI 根据卧室空间、床头墙、门窗衣柜和动线选择合适位置"),
    facing: toReadableText(plan.facing, "根据床头墙、窗户、衣柜和用户要求确定朝向"),
    ...
    avoid: toReadableList(plan.avoid, ["不要遮挡门、窗、衣柜、主要通道、采光和床头柜/插座位置"]),
...
function createMockSvgDataUrl(perspective: string, ratio: string): string {
  ...
  <text x="700" y="805" text-anchor="middle" font-family="Arial, sans-serif" font-size="34" fill="#334155">AI 床具摆放 · ${escapeXml(perspective)} · ${escapeXml(ratio)}</text>
</svg>`;
