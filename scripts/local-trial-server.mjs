...
async function generateSeedreamImages(body, apiKey) {
  ...
  const masterImage = await requestSeedreamImage(apiKey, model, masterPrompt, buildSeedreamImageInputs(body), size, "wide");
...
async function requestSeedreamImage(apiKey, model, prompt, images, size, perspective) {
...
function normalizeSeedreamSize(clarity) {
...
function parseSeedreamError(raw) {
  try {
    const data = JSON.parse(raw);
