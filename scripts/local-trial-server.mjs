...
async function handleGemini(req, res) {
  ...
  if (provider === "ark") {
    ...
    if (!apiKey) {
      sendJson(res, 500, {
        success: false,
        message: "未配置 ARK_API_KEY，当前无法调用火山方舟。请在 .env.local 或当前终端环境变量中配置后重启服务。"
      });
      return;
    }
  ...
  if (body.mode === "generate" && provider === "seedream") {
    ...
    if (!apiKey) {
      sendJson(res, 500, {
        success: false,
        message: "未配置 ARK_API_KEY，当前无法调用 Seedream。请在 .env.local 或当前终端环境变量中配置后重启服务。"
      });
      return;
    }
  ...
  if (body.mode === "cutout" || body.mode === "erase") {
    ...
    if (!response.ok) throw toGeminiUpstreamError(response.status, raw, body.mode === "erase" ? "原场景清场失败" : "床具前景提取失败");
    const image = extractInteractionImage(JSON.parse(raw));
    if (!image) throw new Error(body.mode === "erase" ? "Gemini 未返回可用的干净场景图" : "Gemini 未返回可用的床具前景图");
    sendJson(res, 200, { success: true, images: [{ perspective: "wide", title: body.mode === "erase" ? "干净场景" : "床具前景", imageUrl: `data:${image.mimeType};base64,${image.data}` }] });
    return;
  }
...
async function handleArkProvider(body, apiKey) {
  if (body.mode === "analyze") {
...
async function requestArkVisionJson(body, apiKey) {
...
function buildArkVisionContent(body) {
  return [
...
function buildArkVisionImages(body) {
  return [
...
async function requestArkEditImage(body, apiKey, model) {
  ...
  return requestArkImageGeneration(apiKey, model, prompt, [image], normalizeArkImageSize(body.settings?.clarity), body.mode);
}
...
async function generateImagesWithInteractions(body, apiKey, model) {
  ...
  if (!masterImage) console.error("[local-trial-server] Gemini image response had no extractable image", summarizeGeminiShape(masterData));
  if (!masterImage) throw new Error("Gemini 未返回可用的摆放主图");
  const results = [{ perspective: "wide", title: "远景（卧室全景）", imageUrl: `data:${masterImage.mimeType};base64,${masterImage.data}` }];
  await Promise.all(requested.filter((item) => item !== "wide").map(async (perspective) => {
    ...
    results.push({ perspective, title: perspective === "medium" ? "中近景" : "近景（床具细节）", imageUrl: `data:${variationImage.mimeType};base64,${variationImage.data}` });
  }));
  results.sort((left, right) => requested.indexOf(left.perspective) - requested.indexOf(right.perspective));
  return results;
}

async function generateSeedreamImages(body, apiKey) {
  const requested = body.settings?.perspectives?.length ? body.settings.perspectives : ["medium"];
  ...
  const masterImage = await requestArkImageGeneration(apiKey, model, masterPrompt, buildSeedreamImageInputs(body), size, "wide");
  ...
  await Promise.all(requested.filter((item) => item !== "wide").map(async (perspective) => {
    ...
    const imageUrl = await requestArkImageGeneration(
      ...
      model,
      prompt,
      [masterImage, ...buildSeedreamProductInputs(body)],
      size,
      perspective
    );
...
async function requestArkImageGeneration(apiKey, model, prompt, images, size, perspective) {
...
function buildSeedreamImageInputs(body) {
  return [
    imageToDataUrl(body.roomImage),
    ...(body.roomReferenceImages || []).map(imageToDataUrl),
    imageToDataUrl(getBeddingImage(body)),
...
function buildSeedreamProductInputs(body) {
  return [
    imageToDataUrl(getBeddingImage(body)),
    imageToDataUrl(body.productReferenceImage)
  ].filter(Boolean);
...
function normalizeArkImageSize(clarity) {
...
function extractSeedreamImageUrl(data) {
...
function extractArkText(data) {
  ...
  if (Array.isArray(choices)) {
    ...
    if (Array.isArray(content)) {
      return content.map((item) => item?.text).filter((text) => typeof text === "string").join("\n").trim();
    }
  }
  return "";
}

function toGeminiTextResponse(text) {
  return { candidates: [{ content: { parts: [{ text }] } }] };
}

function parseArkError(raw) {
  try {
    const data = JSON.parse(raw);
...
function createMockResponse(body) {
  if (body.mode === "analyze") {
    return {
      success: true,
      analysis: {
        roomSummary: "卧室空间开阔，地面与墙面关系清晰，适合进行床具摆放。",
        beddingSummary: "床具主体清晰，应保留颜色、面料质感、扶手和坐垫结构。",
        lighting: "自然光从侧向进入，需要补充地面接触阴影和环境反射。",
        perspective: "卧室具备明显纵深，床具应随地面透视线调整尺度和角度。",
        placementAdvice: "建议优先靠墙或靠窗一侧试摆，避免遮挡主要通行动线。",
        constraints: ["不要改变卧室主体结构", "不要添加人物和文字", "保持原床具材质与比例"],
        placementPlan: {
          summary: "将目标床具自然试摆到卧室核心会客区，优先保证通行与视觉平衡。",
...
function parseAnalysis(data) {
  ...
  try {
    ...
    return {
      roomSummary: toReadableText(parsed.roomSummary, "已识别卧室空间与地面关系。"),
      beddingSummary: toReadableText(parsed.beddingSummary, "已识别床具主体、材质和外观。"),
      lighting: toReadableText(parsed.lighting, "已判断主要光线方向。"),
      perspective: toReadableText(parsed.perspective, "已判断卧室透视。"),
      placementAdvice: toReadableText(parsed.placementAdvice, "建议按空间动线自然摆放。"),
      constraints: toReadableList(parsed.constraints, ["保持卧室主体结构不变"]),
      placementPlan: parsePlacementPlan(parsed.placementPlan, parsed.placementAdvice)
    };
  } catch {
    return {
      roomSummary: text || "已识别卧室空间与地面关系。",
      beddingSummary: "已识别床具主体、材质和外观。",
      lighting: "已判断主要光线方向。",
      perspective: "已判断卧室透视。",
      placementAdvice: "建议按空间动线自然摆放。",
      constraints: ["保持卧室主体结构不变"],
      placementPlan: parsePlacementPlan(null, "根据空间动线自然摆放目标床具。")
    };
  }
}
...
function parsePlacementPlan(value, fallbackAdvice) {
  ...
  const fallback = toReadableText(fallbackAdvice, "根据空间动线自然摆放目标床具。");
  return {
    summary: toReadableText(plan.summary, fallback),
    placement: toReadableText(plan.placement, "由 AI 根据卧室空间、动线和视觉焦点选择合适位置"),
    facing: toReadableText(plan.facing, "根据主要视觉焦点和用户要求确定朝向"),
    scale: toReadableText(plan.scale, "保持与卧室尺度和透视关系协调"),
...
function parseCandidates(value) {
  ...
  return candidates.map((candidate, index) => {
    ...
    return {
      ...
      scale: toReadableText(item.scale, "按卧室透视协调缩放"),
...
function createMockSvgDataUrl(perspective, ratio) {
  ...
  <text x="700" y="805" text-anchor="middle" font-family="Arial, sans-serif" font-size="34" fill="#334155">AI 别墅床具摆放 · ${escapeXml(perspective)} · ${escapeXml(ratio)}</text>
</svg>`;
