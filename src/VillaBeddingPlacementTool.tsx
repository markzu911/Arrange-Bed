...
type GuidedStep = "room" | "bedding" | "review" | "generating" | "result";
...
export function VillaBeddingPlacementTool() {
  ...
  const [isAnalyzingBedding, setIsAnalyzingBedding] = useState(false);
  ...
  const isStandaloneTrial = platform.userId === "demo-user" && platform.toolId === "villa-bedding-placement";
  ...
  const guideCopy = useMemo(() => {
    ...
    if (guidedStep === "review") {
      return {
        eyebrow: "第 3 步",
        title: "确认摆放方案",
        desc: "我已经整理好空间和床具分析。您只需要确认位置、视角和比例即可生成。",
  ...
  function submitChatNote() {
    ...
    if (/^(你好|嗨|hi|hello)/i.test(note)) {
      addChatMessage({ role: "assistant", text: "你好，很高兴帮您做床具摆放。您可以问我能做什么，或直接说“开始摆放”。" });
      return;
    }

    if (/(能做什么|可以干嘛|做什么|功能|怎么用)/.test(note)) {
      addChatMessage({ role: "assistant", text: "我能根据一张卧室照片和一张床具照片，生成床具在卧室里的真实摆放效果。流程是：上传卧室，自动分析；上传床具，自动匹配；确认方案后生成效果图。准备好后对我说“开始摆放”。" });
      return;
    }
    ...
    if (agentFlowStarted) {
      ...
      addChatMessage({
        ...
        text: guidedStep === "review"
          ...
          : "收到，我已记下这条摆放要求，后续会与卧室和床具照片一起用于合成。"
      });
      return;
    }
  }
  ...
  function startVirtualRoomFlow() {
    ...
    setStatus(`已选择 ${styleLabel} 虚拟卧室，请上传床具照片`);
  ...
  async function autoAnalyzeRoom(nextRoomImage: UploadedImage) {
    ...
    try {
      ...
      setGuidedStep("bedding");
      addChatMessage({ role: "assistant", text: "卧室分析暂时没有完成，但我们仍可继续。请上传床具照片，我会在生成时重新匹配。" });
    } finally {
      setIsAnalyzingRoom(false);
    }
  }

  async function autoAnalyzeBedding(nextBeddingImage: UploadedImage) {
    if (useVirtualRoom) {
      const styleLabel = virtualRoomStyleLabels[settings.virtualRoomStyle];
      setIsAnalyzingBedding(true);
      setStatus("正在准备虚拟卧室摆放方案...");
      try {
        const nextAnalysis = createVirtualRoomAnalysis(styleLabel);
        setAnalysis(nextAnalysis);
        setBeddingForegroundImage(nextBeddingImage);
        setClearedRoomImage(null);
        setReviewSubstep("plan");
    ...
    try {
      ...
      setBeddingForegroundImage(null);
      setClearedRoomImage(null);
      setStatus("床具前景或场景清场失败，请重新上传或重试；系统不会使用原图继续摆放");
      addChatMessage({ role: "assistant", text: "床具前景提取或原场景清场没有完成。为避免生成出错误床具或不同卧室，系统已停止后续摆放，请重新上传清晰图片后重试。" });
    } finally {
      setIsAnalyzingBedding(false);
    }
  }
  ...
  async function handleGenerate(correctionPrompt = "") {
    if (!beddingImage || !analysis || (!useVirtualRoom && (!roomImage || !beddingForegroundImage || !clearedRoomImage))) {
      setError("请先完成卧室和床具上传");
      return;
    }
    ...
    try {
      ...
      const images = useVirtualRoom
        ...
        : await generatePlacementImages(clearedRoomImage as UploadedImage, beddingForegroundImage as UploadedImage, beddingImage, [], analysis, generationSettings, platform.context, platform.prompt);
      ...
      if (!useVirtualRoom && generated.length && roomImage) {
        setStatus("摆放效果已生成，正在自动检查是否符合确认方案...");
        try {
          ...
          setStatus(issues.length ? "摆放效果已生成，请查看自动检查建议" : "摆放效果已生成并通过自动检查");
          if (issues.length) {
            addChatMessage({ role: "assistant", text: `自动检查发现：${issues.join("；")}。您可以返回调整方案后重新生成。` });
          }
        } catch {
          setStatus("摆放效果已生成，自动检查暂不可用，请人工确认效果");
        }
      }
    } catch (err) {
      if (isInsufficientIntegralError(err)) {
  ...
  async function refreshPlacementPlan() {
    if (!beddingImage) return;
    await autoAnalyzeBedding(beddingImage);
  }
  ...
  const currentStepContent = (
    <>
      ...
      {guidedStep === "bedding" && (
        <div className={styles.focusLayout}>
          ...
          <UploadStep
            kind="bedding"
            image={beddingImage}
            busy={isAnalyzingBedding}
            title="上传床具照片"
            description="上传后自动合并卧室和床具分析，生成摆放建议。"
            onFile={(file) => handleUpload("bedding", file)}
          />
        </div>
      )}
      ...
      {guidedStep === "review" && analysis && (
        <ReviewStep
          analysis={analysis}
          beddingForegroundImage={beddingForegroundImage}
          settings={settings}
          ...
          isRefreshingPlan={isAnalyzingBedding}
        />
      )}
      ...
      {guidedStep === "generating" && (
        <section className={styles.generatingCard}>
          ...
          <h3>正在生成床具摆放效果</h3>
          <p>我正在匹配卧室透视、床具尺度、光照和地面阴影。完成后会自动进入结果页。</p>
        </section>
      )}
    ...
    <main className={styles.toolShell}>
      ...
      {mode === "agent" ? (
        <ChatWorkspace
          ...
          draft={chatDraft}
          status={isAnalyzingRoom || isAnalyzingBedding || isGenerating ? status : ""}
          error={error}
  ...
  return (
    <section className={styles.virtualRoomPanel}>
      <div className={styles.virtualRoomHeader}>
        <Wand2 size={20} />
        <div>
          <strong>跳过卧室，生成虚拟卧室</strong>
          <span>选择装修风格后，只上传床具即可生成虚拟卧室效果。</span>
        </div>
      </div>
  ...
  return (
    <section className={styles.virtualRoomPanel}>
      <div className={styles.virtualRoomHeader}>
        <Wand2 size={20} />
        <div>
          <strong>{virtualRoomStyleLabels[selectedStyle]}虚拟卧室</strong>
          <span>已跳过真实卧室上传。接下来上传床具，AI 会围绕同一虚拟卧室生成远景、中近景和近景。</span>
        </div>
      </div>
    </section>
  );
...
function ReviewStep({
  ...
  beddingForegroundImage: UploadedImage | null;
            ...
            <div className={styles.planOverview}>
              ...
              <section className={styles.planCard}>
                ...
                <div className={styles.planActions}>
                  ...
                  {beddingForegroundImage && <span>床具前景已锁定</span>}
                </div>
          ...
          <>
            ...
            {showAnalysisEditor && (
              <div className={styles.analysisEditor}>
                <AnalysisField label="卧室分析" value={analysis.roomSummary} onChange={(value) => onAnalysisChange("roomSummary", value)} />
                <AnalysisField label="床具分析" value={analysis.beddingSummary} onChange={(value) => onAnalysisChange("beddingSummary", value)} />
