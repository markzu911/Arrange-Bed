...
export function buildGenerationPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    wide: "镜头必须为远景：相机退到卧室入口、门侧或房间角落，以较低或正常站立高度拍摄，展示整张床以及大部分卧室环境，包括床头墙、窗、门、衣柜、主要通道、地面和部分天花关系；目标床具在画面中占比约 22% 到 38%，床必须完整可见，环境信息要充足。",
    medium: "镜头必须为中近景：相机前移到床区边缘，可从床左前方、右前方或略低机位拍摄，角度相对远景有明显变化；以床区为核心，不要求展示整张床，可以自然裁掉床尾、床侧或部分床头，只需保留足够床体与床周环境来理解摆放关系，目标床具在画面中占比约 42% 到 65%。",
    close: "镜头必须为近景：相机贴近目标床具的局部产品细节，可从侧前方、床头斜侧、床沿低机位或俯视小角度拍摄；不需要展示整张床，重点是床品材质、纹理、缝线、软包、床垫厚度、靠枕/被面层次和产品工艺细节，仅保留少量环境作为氛围参照。"
  };
...
export function buildCameraVariationPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    medium: "中近景：相机从远景位置明显前移到床区边缘，并向床左前方或右前方横移 20 到 45 度，也可以略微降低或抬高相机高度，形成可见侧向视差；不要求展示整张床，可以自然裁掉床尾、床侧或部分床头，目标床具占画面约 42% 到 65%。只保留床头墙、床头柜、地毯/地面、窗或衣柜中的 1 到 2 个参照物来证明仍是同一卧室。",
    close: "近景：相机贴近目标床具局部，可以从床头斜侧、床沿低机位、被面上方小俯视或侧前方拍摄；不需要展示整张床，重点展示床品产品细节，包括材质纹理、缝线、软包、床垫厚度、靠枕/被面层次、边角工艺和触感，只保留局部地面、墙面、床头柜或光影作为环境上下文。"
  };
  const cameraDeltaInstruction: Record<string, string> = {
    medium: "中近景与远景的差异必须一眼可见：相机距离、左右角度或高度至少有一项明显不同，床具主体面积至少达到远景的 1.6 倍，画面边缘必须少于远景的全卧室信息；允许床不完整，但床区关系要清楚。",
    close: "近景与远景的差异必须非常明显：相机贴近产品局部，画面重点从卧室展示转为床品细节展示；床可以只出现局部，远端空间大部分不可见，不能再追求完整床体展示。"
  };
  return [
    ...
    "禁止输出仅裁切、缩放或放大主图得到的画面。中近景和近景不能只是远景裁切或放大，必须是真实相机位移；中近景允许床体不完整，近景应主动转向产品细节，而不是继续展示完整床和完整房间。",
...
export function buildVirtualRoomPrompt(
  ...
  const cameraInstruction: Record<string, string> = {
    wide: "远景：相机位于虚拟卧室入口、门侧或角落，展示整张床以及大部分卧室环境，包括床头墙、窗、衣柜、门、通道、地面和软装关系；目标床具占画面约 22% 到 38%，必须能看清它在空间中的完整摆放。",
    medium: "中近景：相机明显靠近床区并改变左右角度或高度，可从床左前方、右前方、略低机位或轻微俯视拍摄；不要求展示整张床，可以裁掉床尾、床侧或部分床头，保留床周边的床头墙、床头柜、地毯/地面、窗景或衣柜作为空间参照。",
    close: "近景：相机贴近目标床具局部产品细节，可从床头斜侧、床沿低机位、被面上方小俯视或侧前方拍摄；不需要展示整张床，重点展示材质纹理、缝线、软包、床垫厚度、靠枕/被面层次、边角工艺和产品触感，仅保留少量虚拟卧室上下文。"
  };
  ...
  return [
    ...
    "多视角生成时，同一个床具、同一个卧室、同一个摆放方案必须保持一致。远景负责展示床和大部分环境，中近景负责展示床区并允许床体不完整，近景负责展示产品细节。三种视角可以使用不同相机高度、左右角度和拍摄距离，不能只是远景裁切或放大。",
    extraContext ? `平台传入上下文：${extraContext}` : "",
    extraPrompt.length ? `平台补充关键词：${extraPrompt.join("、")}` : ""
  ].filter(Boolean).join("\n");
}
